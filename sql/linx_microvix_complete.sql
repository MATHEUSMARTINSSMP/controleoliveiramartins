-- =============================================================================
-- INTEGRAÇÃO COMPLETA LINX MICROVIX
...
-- #############################################################################
-- PARTE 1: HUB FIDELIDADE (CASHBACK/PONTOS)
-- #############################################################################

-- -----------------------------------------------------------------------------
-- 1.1 TABELA: linx_microvix_config
-- Configurações de integração Linx Microvix por loja (multi-tenant)
-- Armazena credenciais do Hub Fidelidade E WebService de Saída
-- -----------------------------------------------------------------------------
...
CREATE TABLE sistemaretiradas.linx_microvix_config (
    ...
    -- Controle de timestamp (incrementa do SQL Server, não UNIX)
    ...
    -- Constraint: apenas uma config por loja
    CONSTRAINT linx_microvix_config_unique_store UNIQUE (store_id)
);

-- Índices
CREATE INDEX idx_linx_microvix_config_store ON sistemaretiradas.linx_microvix_config(store_id);
CREATE INDEX idx_linx_microvix_config_cnpj ON sistemaretiradas.linx_microvix_config(cnpj);
CREATE INDEX idx_linx_microvix_config_active ON sistemaretiradas.linx_microvix_config(active);

-- RLS
ALTER TABLE sistemaretiradas.linx_microvix_config ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "linx_microvix_config_tenant_isolation" ON sistemaretiradas.linx_microvix_config;
...
DROP POLICY IF EXISTS "linx_microvix_config_service_role" ON sistemaretiradas.linx_microvix_config;
CREATE POLICY "linx_microvix_config_service_role" ON sistemaretiradas.linx_microvix_config
    FOR ALL USING (auth.role() = 'service_role');

-- Trigger para updated_at
CREATE OR REPLACE FUNCTION sistemaretiradas.update_linx_microvix_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER trigger_linx_microvix_config_updated_at
    ...
    FOR EACH ROW
    EXECUTE FUNCTION sistemaretiradas.update_linx_microvix_updated_at();

-- -----------------------------------------------------------------------------
-- 1.2 TABELA: linx_microvix_parceiros
-- Cache de parceiros de fidelidade (obtidos via BuscarParceiros)
-- -----------------------------------------------------------------------------
...
CREATE INDEX idx_linx_microvix_clientes_store ON sistemaretiradas.linx_microvix_clientes(store_id);
CREATE INDEX idx_linx_microvix_clientes_cpf ON sistemaretiradas.linx_microvix_clientes(cpf);
...
CREATE INDEX idx_linx_microvix_transacoes_store ON sistemaretiradas.linx_microvix_transacoes(store_id);
CREATE INDEX idx_linx_microvix_transacoes_cpf ON sistemaretiradas.linx_microvix_transacoes(cpf_cliente);
CREATE INDEX idx_linx_microvix_transacoes_tipo ON sistemaretiradas.linx_microvix_transacoes(tipo_operacao);
CREATE INDEX idx_linx_microvix_transacoes_status ON sistemaretiradas.linx_microvix_transacoes(status);
CREATE INDEX idx_linx_microvix_transacoes_created ON sistemaretiradas.linx_microvix_transacoes(created_at);
...
CREATE INDEX idx_linx_microvix_campanhas_store ON sistemaretiradas.linx_microvix_campanhas(store_id);
CREATE INDEX idx_linx_microvix_campanhas_active ON sistemaretiradas.linx_microvix_campanhas(active);
...
CREATE INDEX idx_linx_microvix_bonus_store ON sistemaretiradas.linx_microvix_bonus(store_id);
CREATE INDEX idx_linx_microvix_bonus_cpf ON sistemaretiradas.linx_microvix_bonus(cpf_cliente);
CREATE INDEX idx_linx_microvix_bonus_tipo ON sistemaretiradas.linx_microvix_bonus(tipo);
CREATE INDEX idx_linx_microvix_bonus_created ON sistemaretiradas.linx_microvix_bonus(created_at);
...
-- #############################################################################
-- PARTE 2: WEBSERVICE DE SAÍDA (SINCRONIZAÇÃO DE VENDAS)
-- #############################################################################

-- -----------------------------------------------------------------------------
-- 2.1 TABELA: linx_microvix_vendas
-- Vendas sincronizadas do Microvix (similar a orders do Tiny)
-- -----------------------------------------------------------------------------
CREATE TABLE sistemaretiradas.linx_microvix_vendas (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    store_id UUID NOT NULL REFERENCES sistemaretiradas.stores(id) ON DELETE CASCADE,
    
    -- Identificadores Linx
    ...
    -- Timestamp Linx (para controle de sync)
    ...
    CONSTRAINT linx_microvix_vendas_unique UNIQUE (store_id, id_transacao)
);

CREATE INDEX idx_linx_microvix_vendas_store ON sistemaretiradas.linx_microvix_vendas(store_id);
CREATE INDEX idx_linx_microvix_vendas_data ON sistemaretiradas.linx_microvix_vendas(data_venda);
CREATE INDEX idx_linx_microvix_vendas_cliente ON sistemaretiradas.linx_microvix_vendas(doc_cliente);
CREATE INDEX idx_linx_microvix_vendas_vendedor ON sistemaretiradas.linx_microvix_vendas(cod_vendedor);
CREATE INDEX idx_linx_microvix_vendas_timestamp ON sistemaretiradas.linx_microvix_vendas(timestamp_linx);
CREATE INDEX idx_linx_microvix_vendas_situacao ON sistemaretiradas.linx_microvix_vendas(situacao);

ALTER TABLE sistemaretiradas.linx_microvix_vendas ENABLE ROW LEVEL SECURITY;

CREATE POLICY "linx_microvix_vendas_tenant_isolation" ON sistemaretiradas.linx_microvix_vendas
    FOR ALL USING (
        store_id IN (
            SELECT id FROM sistemaretiradas.stores 
            WHERE tenant_id = (
...
CREATE POLICY "linx_microvix_vendas_service_role" ON sistemaretiradas.linx_microvix_vendas
    FOR ALL USING (auth.role() = 'service_role');

DROP TRIGGER IF EXISTS trigger_linx_microvix_vendas_updated_at ON sistemaretiradas.linx_microvix_vendas;
CREATE TRIGGER trigger_linx_microvix_vendas_updated_at
...
-- Itens das vendas sincronizadas
-- -----------------------------------------------------------------------------
...
CREATE INDEX idx_linx_microvix_vendas_itens_venda ON sistemaretiradas.linx_microvix_vendas_itens(venda_id);
CREATE INDEX idx_linx_microvix_vendas_itens_store ON sistemaretiradas.linx_microvix_vendas_itens(store_id);
CREATE INDEX idx_linx_microvix_vendas_itens_produto ON sistemaretiradas.linx_microvix_vendas_itens(cod_produto);

ALTER TABLE sistemaretiradas.linx_microvix_vendas_itens ENABLE ROW LEVEL SECURITY;

CREATE POLICY "linx_microvix_vendas_itens_tenant_isolation" ON sistemaretiradas.linx_microvix_vendas_itens
    FOR ALL USING (
        store_id IN (
            SELECT id FROM sistemaretiradas.stores 
            WHERE tenant_id = (
...
CREATE POLICY "linx_microvix_vendas_itens_service_role" ON sistemaretiradas.linx_microvix_vendas_itens
    FOR ALL USING (auth.role() = 'service_role');

-- -----------------------------------------------------------------------------
-- 2.3 TABELA: linx_microvix_sync_log
-- Log de sincronizações do WebService
-- -----------------------------------------------------------------------------
...
CREATE INDEX idx_linx_microvix_sync_log_store ON sistemaretiradas.linx_microvix_sync_log(store_id);
CREATE INDEX idx_linx_microvix_sync_log_tipo ON sistemaretiradas.linx_microvix_sync_log(tipo_sync);
CREATE INDEX idx_linx_microvix_sync_log_status ON sistemaretiradas.linx_microvix_sync_log(status);
CREATE INDEX idx_linx_microvix_sync_log_started ON sistemaretiradas.linx_microvix_sync_log(started_at);

ALTER TABLE sistemaretiradas.linx_microvix_sync_log ENABLE ROW LEVEL SECURITY;

CREATE POLICY "linx_microvix_sync_log_tenant_isolation" ON sistemaretiradas.linx_microvix_sync_log
    FOR ALL USING (
        store_id IN (
            SELECT id FROM sistemaretiradas.stores 
            WHERE tenant_id = (
...
CREATE POLICY "linx_microvix_sync_log_service_role" ON sistemaretiradas.linx_microvix_sync_log
    FOR ALL USING (auth.role() = 'service_role');
...
COMMENT ON TABLE sistemaretiradas.linx_microvix_config IS 'Configurações de integração Linx Microvix (Hub Fidelidade + WebService de Saída)';
