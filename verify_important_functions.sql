...
SELECT 
    ...
    CASE 
        WHEN proname IS NULL THEN '❌ FALTANDO'
        ELSE '✅ EXISTE'
    END as status
FROM (
    SELECT 'enqueue_cashback_whatsapp' as proname
...
ORDER BY categoria, function_name;
...
SELECT 
    ...
    END as status
FROM (
    SELECT 'gerar_cashback' as proname
...
SELECT 
    ...
    END as status
FROM (
    SELECT 'chamar_sync_tiny_orders' as proname
) expected
LEFT JOIN pg_proc p ON p.proname = expected.proname
    AND p.pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'sistemaretiradas')
ORDER BY categoria, function_name;
...
SELECT 
    ...
    END as status
FROM (
    SELECT 'process_store_task_alerts' as proname
...
SELECT 
    ...
    END as status
FROM (
    SELECT 'validate_store_notification_limit' as proname
...
SELECT 
    ...
    proname as function_name,
    CASE 
        WHEN proname IS NULL THEN '✅ CORRETO (não existe)'
        ELSE '❌ AINDA EXISTE (deve ser removida)'
    END as status
FROM (
    SELECT 'validate_notification_limit' as proname
