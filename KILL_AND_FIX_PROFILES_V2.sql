-- SCRIPT DE EMERGÊNCIA CORRIGIDO (COM CASCADE)
...
-- 2. Remover a função e TUDO que depende dela (Políticas) de uma vez
-- O CASCADE é a chave aqui: ele remove as políticas que travavam o drop
DROP FUNCTION IF EXISTS sistemaretiradas.get_user_role() CASCADE;

-- 3. Remover quaisquer outras políticas que tenham sobrado (limpeza extra)
DO $$
DECLARE
    r RECORD;
BEGIN
...
SELECT 'SUCESSO: Função e políticas recriadas com CASCADE.' as status;
