...
https://fluxos.eleveaagencia.com.br/webhook/api/auth/google/callback  (antigo n8n - manter para rollback)
```
...
#### ✅ Migrado para Netlify Functions:
- **Start URL**: `https://eleveaone.com.br/.netlify/functions/google-oauth-start`
- **Callback URL**: `https://eleveaone.com.br/.netlify/functions/google-oauth-callback`
- **PKCE**: Implementado corretamente ✅

**📚 Guia completo de configuração:** Veja `docs/CONFIGURACAO_GOOGLE_OAUTH_NETLIFY.md`
...
**📚 Guia completo:** Veja `docs/CONFIGURACAO_GOOGLE_OAUTH_NETLIFY.md`

#### Passo 2: Configurar URIs no Google Cloud Console
...
3. Em **"URIs de redirecionamento autorizados"**, adicione (clicando em "+ Adicionar URI"):
   - `https://eleveaone.com.br/.netlify/functions/google-oauth-callback` (NOVO - Netlify Functions)
   - `https://fluxos.eleveaagencia.com.br/webhook/api/auth/google/callback` (ANTIGO - n8n - manter para rollback)
   
   ⚠️ **IMPORTANTE**: A URI deve ser EXATAMENTE igual à configurada, sem barra no final.
...
#### Passo 4: Verificar Escopos Sensíveis
...
1. **Teste o endpoint de start:**
   ```
   GET https://eleveaone.com.br/.netlify/functions/google-oauth-start?customerId=TEST&siteSlug=test
   ```
...
6. **A Netlify Function processará o callback** e trocará o código por tokens

7. **Verifique os logs do Netlify** para confirmar que tudo funcionou (Netlify Dashboard → Functions → Logs)
...
### 8. Configuração no Netlify (Variáveis de Ambiente)

**✅ Já configurado**: As credenciais estão sendo lidas de variáveis de ambiente no Netlify.

**Variáveis necessárias:**
- `GOOGLE_CLIENT_ID`: Client ID do Google OAuth
- `GOOGLE_CLIENT_SECRET`: Client Secret do Google OAuth
- `GOOGLE_REDIRECT_URI`: URI de callback (deve ser igual ao configurado no Google Cloud Console)
- `FRONTEND_URL`: URL do frontend (para redirecionamento após OAuth)
