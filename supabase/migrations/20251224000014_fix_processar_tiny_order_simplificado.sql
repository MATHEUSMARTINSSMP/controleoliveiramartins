-- ============================================================================
-- CORREÇÃO SIMPLIFICADA: FUNÇÃO processar_tiny_order_para_venda
-- ============================================================================
-- Data: 2025-12-24
-- Problema: Função está retornando NULL, possivelmente por tentar usar colunas
--           que não existem ou constraints que falham
-- Solução: Versão simplificada que só usa colunas essenciais primeiro
-- ============================================================================

-- VERSÃO SIMPLIFICADA DA FUNÇÃO (SEM COLUNAS OPCIONAIS)
-- ============================================================================
-- Esta versão só usa as colunas mais básicas para garantir que funcione
...
DECLARE
  ...
  v_observacoes TEXT;
BEGIN
  -- Buscar dados do pedido (apenas campos essenciais primeiro)
  SELECT 
    id,
    store_id,
    colaboradora_id,
  ...
  IF v_sale_id IS NOT NULL THEN
    ...
    RETURN v_sale_id;
  ELSE
    -- Criar nova venda (apenas campos essenciais)
    BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING '[processar_tiny_order_para_venda] ❌ Erro ao criar venda: %', SQLERRM;
      RETURN NULL;
    END;
  END IF;
...
VERSÃO SIMPLIFICADA: Usa apenas colunas essenciais para garantir compatibilidade.
Usado pelo trigger automático para processar pedidos em tempo real.';

-- TESTAR A FUNÇÃO COM UM DOS PEDIDOS PENDENTES
-- ============================================================================
