...
CREATE INDEX IF NOT EXISTS idx_google_reviews_customer_site_read_date 
  ON sistemaretiradas.google_reviews(customer_id, site_slug, is_read, review_date DESC)
...
CREATE INDEX IF NOT EXISTS idx_google_reviews_customer_site_rating_date 
  ON sistemaretiradas.google_reviews(customer_id, site_slug, rating, review_date DESC);
...
CREATE INDEX IF NOT EXISTS idx_google_reviews_customer_site_account_location 
  ON sistemaretiradas.google_reviews(customer_id, site_slug, account_id, location_id)
...
CREATE INDEX IF NOT EXISTS idx_google_reviews_customer_site_reply 
  ON sistemaretiradas.google_reviews(customer_id, site_slug, (CASE WHEN reply IS NULL OR reply = '' THEN false ELSE true END), review_date DESC);
...
-- Comentários
COMMENT ON INDEX sistemaretiradas.idx_google_reviews_customer_site_read_date IS 
'Índice para queries de reviews não lidas ordenadas por data (filtros comuns)';

COMMENT ON INDEX sistemaretiradas.idx_google_reviews_customer_site_rating_date IS 
'Índice para queries filtradas por rating e ordenadas por data';

COMMENT ON INDEX sistemaretiradas.idx_google_reviews_customer_site_account_location IS 
'Índice para queries filtradas por account e location específicos';

COMMENT ON INDEX sistemaretiradas.idx_google_reviews_customer_site_reply IS 
'Índice para queries filtradas por reviews respondidas/não respondidas';

COMMENT ON INDEX sistemaretiradas.idx_google_reviews_comment_text IS 
