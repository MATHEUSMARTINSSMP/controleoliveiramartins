...
-- Adicionar campos para suportar clientes (não só colaboradoras)
DO $$
BEGIN
...
-- Adicionar suporte para clientes em cashback_balance
DO $$
BEGIN
    -- Adicionar cliente_id (FK para tiny_contacts)
    IF NOT EXISTS (
...
CREATE TRIGGER trigger_calculate_cashback_tiny_order
    ...
    WHEN (
        ...
        AND (
            -- INSERT: sempre processar se for faturado/aprovado
            TG_OP = 'INSERT' OR
            -- UPDATE: processar se situacao mudou para faturado/aprovado
            (TG_OP = 'UPDATE' AND OLD.situacao NOT IN ('1', '3') AND NEW.situacao IN ('1', '3'))
        )
    )
