...
-- ============================================================================
CREATE TABLE IF NOT EXISTS sistemaretiradas.google_credentials (
...
-- Comentários
COMMENT ON TABLE sistemaretiradas.google_credentials IS 'Credenciais OAuth do Google por customer e site';
COMMENT ON COLUMN sistemaretiradas.google_credentials.customer_id IS 'ID do cliente (email)';
COMMENT ON COLUMN sistemaretiradas.google_credentials.site_slug IS 'Slug do site';
COMMENT ON COLUMN sistemaretiradas.google_credentials.scopes IS 'Escopos OAuth autorizados';
COMMENT ON COLUMN sistemaretiradas.google_credentials.status IS 'Status: active, expired, revoked';
COMMENT ON COLUMN sistemaretiradas.google_credentials.access_token IS 'Access token OAuth';
COMMENT ON COLUMN sistemaretiradas.google_credentials.refresh_token IS 'Refresh token OAuth';
COMMENT ON COLUMN sistemaretiradas.google_credentials.expires_at IS 'Data de expiração do access_token';

-- Índices
CREATE INDEX IF NOT EXISTS idx_google_credentials_status 
  ON sistemaretiradas.google_credentials(status);
CREATE INDEX IF NOT EXISTS idx_google_credentials_expires_at 
  ON sistemaretiradas.google_credentials(expires_at);

-- ============================================================================
-- Tabela: google_reviews
-- Armazena reviews do Google My Business
-- ============================================================================
CREATE TABLE IF NOT EXISTS sistemaretiradas.google_reviews (
...
-- Comentários
COMMENT ON TABLE sistemaretiradas.google_reviews IS 'Reviews do Google My Business';
COMMENT ON COLUMN sistemaretiradas.google_reviews.review_id_external IS 'ID externo do review (do Google)';
COMMENT ON COLUMN sistemaretiradas.google_reviews.rating IS 'Avaliação de 1 a 5 estrelas';
COMMENT ON COLUMN sistemaretiradas.google_reviews.reply IS 'Resposta ao review';

-- Índices
CREATE INDEX IF NOT EXISTS idx_google_reviews_customer_site 
  ON sistemaretiradas.google_reviews(customer_id, site_slug);
CREATE INDEX IF NOT EXISTS idx_google_reviews_review_date 
  ON sistemaretiradas.google_reviews(review_date DESC);
CREATE INDEX IF NOT EXISTS idx_google_reviews_rating 
  ON sistemaretiradas.google_reviews(rating);
...
-- Habilitar RLS
ALTER TABLE sistemaretiradas.google_credentials ENABLE ROW LEVEL SECURITY;
ALTER TABLE sistemaretiradas.google_reviews ENABLE ROW LEVEL SECURITY;
...
-- Função update_updated_at_column já deve existir em sistemaretiradas
-- Se não existir, será criada abaixo
