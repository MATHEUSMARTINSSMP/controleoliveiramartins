...
-- ============================================================================
CREATE TABLE IF NOT EXISTS elevea.google_business_accounts (
...
-- Comentários
COMMENT ON TABLE elevea.google_business_accounts IS 'Accounts e locations do Google My Business por customer e site';
COMMENT ON COLUMN elevea.google_business_accounts.account_id IS 'ID da account do Google (formato: accounts/123456789)';
COMMENT ON COLUMN elevea.google_business_accounts.location_id IS 'ID da location do Google (formato: locations/987654321)';
COMMENT ON COLUMN elevea.google_business_accounts.is_primary IS 'Indica se é a location padrão para este customer/site';

-- Índices
CREATE INDEX IF NOT EXISTS idx_google_business_accounts_customer_site 
  ON elevea.google_business_accounts(customer_id, site_slug);
CREATE INDEX IF NOT EXISTS idx_google_business_accounts_account 
  ON elevea.google_business_accounts(account_id);
CREATE INDEX IF NOT EXISTS idx_google_business_accounts_location 
  ON elevea.google_business_accounts(location_id);
CREATE INDEX IF NOT EXISTS idx_google_business_accounts_primary 
  ON elevea.google_business_accounts(customer_id, site_slug, is_primary) 
...
-- Habilitar RLS
ALTER TABLE elevea.google_business_accounts ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "ADMIN pode ver google_business_accounts de suas lojas"
  ON elevea.google_business_accounts
  FOR SELECT
  TO authenticated
  USING (
    EXISTS (
...
CREATE TRIGGER update_google_business_accounts_updated_at
  BEFORE UPDATE ON elevea.google_business_accounts
  FOR EACH ROW
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE elevea.google_reviews
...
-- Comentários
COMMENT ON COLUMN elevea.google_reviews.account_id IS 'ID da account do Google (formato: accounts/123456789)';
COMMENT ON COLUMN elevea.google_reviews.location_id IS 'ID da location do Google (formato: locations/987654321)';
COMMENT ON COLUMN elevea.google_reviews.is_read IS 'Indica se o review foi lido pelo usuário';

-- Índices adicionais
CREATE INDEX IF NOT EXISTS idx_google_reviews_account_location 
  ON elevea.google_reviews(account_id, location_id);
CREATE INDEX IF NOT EXISTS idx_google_reviews_is_read 
  ON elevea.google_reviews(is_read) 
  WHERE is_read = false;
